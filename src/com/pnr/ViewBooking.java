...
public class ViewBooking extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try{
			...
			String sql = "select pnr_status.pnr_no,pnr_status.train_no,p_name,age,gender,date_of_journey as jdate,status,seat_no,class,A.station_name as source_station_name, A.station_code as source_code,B.station_name as dest_station_name, B.station_code as dest_code,email_id,no_of_tickets,booked_date,total_cost,train_name from pnr_status inner join station as A on( pnr_status.source_code =  A.station_code) inner join station as B on pnr_status.des_code = B.station_code inner join booking  on pnr_status.pnr_no = booking.pnr_no inner join train_details on pnr_status.train_no =  train_details.train_no where booking.email_id ='"+user+"'";
			Statement st =  con.createStatement();
			ResultSet rs =  st.executeQuery(sql);
			PNRModel[] pmodel = new PNRModel[100];
			int i=0;
			while(rs.next())
			{
